...
├── data/
│   └── lessons.py             # LESSONS (36 items), QUIZ_QUESTIONS (15 items)
└── routers/
    ...
    └── papers.py              # GET /papers/, GET /papers/{id} — 14 specs with key_takeaways
```

| Router | Endpoints | Data Source |
|--------|----------|-------------|
| lessons | `GET /`, `GET /{id}`, `GET /categories/summary` | `LESSONS` list (36 items, 12 categories) |
| quiz | `GET /questions`, `POST /check` | `QUIZ_QUESTIONS` list (15 items) |
| papers | `GET /`, `GET /{id}` | `PAPERS` list (14 items with key_takeaways) |
...
├── components/
│   ├── layout/
│   │   └── Sidebar.jsx        # Fixed left nav with links
│   └── ui/
...
└── pages/
    ...
    ├── Simulator.jsx          # Interactive protocol simulation scenarios
    ├── Patterns.jsx           # Protocol pattern catalog with trade-off analysis
    ├── Quiz.jsx               # 15 questions, per-category scoring, Recharts bar chart
    ├── Papers.jsx             # 14 specs with key_takeaways, related_topics, doc info
    └── Settings.jsx           # System status, progress reset, about
```
